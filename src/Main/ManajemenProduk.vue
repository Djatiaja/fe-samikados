  ...
  methods: {
    ...
    async handleSaveProduct(newProduct) {
      ...
      try {
        ...
        formData.append('is_publish', newProduct.is_publish)
        ...
        newProduct.product_variants.forEach((variant, index) => {
          ...
          formData.append(`product_variants[${index}][is_default]`, variant.is_default)
        })
      ...
      try {
        ...
        if (updatedProduct.category !== originalProduct.category) {
          productPayload.category_id =
            this.categories.find((cat) => cat.name === updatedProduct.category)?.id || 1
        }
        ...
        if (updatedProduct.stock_total !== originalProduct.stock_total) {
        ...
        if (
          Object.keys(productPayload).length > 1 ||
          updatedProduct.variations.length ||
          updatedProduct.additionalOptions.length
        ) {
          ...
          if (index !== -1) {
            this.products.splice(index, 1, {
              ...
              stock_total: parseInt(productResponse.data.data.stock_total, 10),
              unit: productResponse.data.data.unit,
              images: [productResponse.data.data.thumbnail_url],
              variant_count: updatedProduct.variations.length,
              finishing_count: updatedProduct.additionalOptions.length,
